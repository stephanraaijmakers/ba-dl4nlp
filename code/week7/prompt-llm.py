...
def Flan():
    ...
    def query_generator(query, options):
        t5query = f"""Question: Is "{query}" grammatical?. Context: * {" * ".join(examples)}"""
        inputs = tokenizer(t5query, return_tensors="pt")
        outputs = model.generate(**inputs, max_new_tokens=20)
        return tokenizer.batch_decode(outputs, skip_special_tokens=True)

    tests = ["a needs water plant", "the today banks early close", "I like sushi"]
    examples = ["Most cars still need petrol:yes", "Most need cars still petrol:no", "I like raisins:yes"]
    for t in tests:
        result = query_generator(t, examples)
...
def Flan2():
    ...
    def query_generator(query):
        t5query = f"""Simplify the following sentence: "{query}"."""
        inputs = tokenizer(t5query, return_tensors="pt")
        outputs = model.generate(**inputs, max_new_tokens=100)
        return tokenizer.batch_decode(outputs, skip_special_tokens=True)

    tests = ["The language generated by LLMs has reached human parity: it is as good as or even better than what most humans would produce"]

    for t in tests:
        result = query_generator(t)
        print(f"{t:<24} {result[0]}")
